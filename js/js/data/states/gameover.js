...
(function(){ 'use strict';
	...
	StateGameover.prototype.init = function() {
		...
		// process clay.io leaderboards and achievements
		var leaderboard = new Clay.Leaderboard( { id: 3839 } );
		leaderboard.post( { score: g.prevState.score }, function( response ) {} );

		if( g.prevState.score >= 50 ) {
			( new Clay.Achievement( { id: 4281 } ) ).award();
		}
		if( g.prevState.score >= 100 ) {
			( new Clay.Achievement( { id: 4282 } ) ).award();
		}
		if( g.prevState.score >= 150 ) {
			( new Clay.Achievement( { id: 4283 } ) ).award();
		}
	};
