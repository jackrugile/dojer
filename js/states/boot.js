...
(function(){ 'use strict';
	...
	StateBoot.prototype.init = function() {
		...
		g.audio.music = new Howl({
			...
			autoplay: true,
			loop: true,
			onload: function(){ _this.loadAudio( _this ) }
		});
		this.total++;
	...
	StateBoot.prototype.step = function() {
		...
		g.text( {
			...
			x: g.s * 7,
			y: g.height / 2 - g.s * 2,
			text: g.util.format.pad( Math.round( ( ( this.loaded / this.total ) * 100 ) ), 2 ) + '%',
			hspacing: g.s,
			vspacing: g.s * 2,
			halign: 'left',
			valign: 'bottom',
		...
		g.ctx.fillStyle = '#000';
		g.ctx.fillRect( 0, g.height / 2 - g.s, g.r( this.loaded / this.total * g.width ), g.s );
		
		g.ctx.fillStyle = '#fff';
		g.ctx.fillRect( 0, g.height / 2, g.r( this.loaded / this.total * g.width ), g.s );
	};
