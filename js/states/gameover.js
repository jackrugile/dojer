...
(function () {
  ...
  StateGameover.prototype.init = function () {
    ...
    g.storage.set(
      "totalScore",
      g.storage.get("totalScore") + g.prevState.score
    );
    ...
    g.storage.set(
      "totalJumps",
      g.storage.get("totalJumps") + g.prevState.tempStorage.jumps
    );

    // update total switches
    g.storage.set(
      "totalSwitches",
      g.storage.get("totalSwitches") + g.prevState.tempStorage.switches
    );
    ...
    g.storage.set(
      "totalTime",
      g.storage.get("totalTime") +
        g.prevState.time.elapsed -
        g.prevState.time.pelapsed
    );
  };
